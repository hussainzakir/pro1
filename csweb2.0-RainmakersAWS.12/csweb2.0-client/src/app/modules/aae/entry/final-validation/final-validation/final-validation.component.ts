...
export class FinalValidationComponent implements OnInit {
  ...
  constructor( private aaeEntryService: AaeEntryService,
    ...
    private dialogRef: MatDialogRef<FinalValidationComponent>) {
  ...
  cancel(){
    ...
    this.aaeEntryService.resetAccountNumber(this.quoteId).subscribe(response => {
  ...
  save(){
    this._router.navigateByUrl('/aae');
    this.errorMsg = null;
    this.loading = true;
